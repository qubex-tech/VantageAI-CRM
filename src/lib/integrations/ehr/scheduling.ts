...
export async function getEcwOpenSlots(params: {
  ...
  timeZone?: string
  syncFirst?: boolean
}): Promise<{ appointments: EcwScheduleAppointment[]; slots: EcwOpenSlot[] }> {
  ...
  const busy = appointments
    .filter((appointment) => encounterBlocksAvailability(appointment.status))
    .map((appointment) => ({
  ...
  while (cursor <= endDay) {
    ...
    if (weekday !== 'Sat' && weekday !== 'Sun') {
      ...
      for (let hour = DEFAULT_BUSINESS_START_HOUR; hour < DEFAULT_BUSINESS_END_HOUR; hour++) {
        for (let minute = 0; minute < 60; minute += lengthMinutes) {
          if (hour === DEFAULT_BUSINESS_END_HOUR - 1 && minute + lengthMinutes > 60) continue
          const localStart = new Date(
            `${dateStr}T${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}:00`
          )
          const startMs = localStart.getTime()
          const endMs = startMs + lengthMinutes * 60_000
          if (startMs < Date.now()) continue
          if (busy.some((interval) => overlaps(startMs, endMs, interval.startMs, interval.endMs))) {
            continue
          }
          slots.push({
...
export async function getEcwScheduleFromSettings(params: {
  ...
  const practitionerRef = params.practitionerRef || resolveReadPractitionerRef(params.scheduling)
  if (!practitionerRef) {
    throw new Error(
      'No eClinicalWorks practitioner configured. Set a default practitioner in Scheduling settings.'
    )
  }
  const lengthMinutes =
