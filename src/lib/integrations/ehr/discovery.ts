...
export async function discoverSmartConfiguration(
  ...
  const issuer = normalizeUrl(issuerInput)
  ...
  if (wellKnownResponse.ok) {
    ...
    return {
      issuer: wellKnown.issuer ? normalizeUrl(wellKnown.issuer) : issuer,
      fhirBaseUrl: normalizeUrl(wellKnown.fhir_base_url || issuer),
