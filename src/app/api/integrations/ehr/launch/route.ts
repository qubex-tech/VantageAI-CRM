...
export async function GET(req: NextRequest) {
  try {
    ...
    const issInput = (transformedParams.iss as string | undefined) || parsed.data.iss || (config as any).issuer
    const launchParam = (transformedParams.launch as string | undefined) || parsed.data.launch
    if (!issInput) {
      return NextResponse.json({ error: 'Missing issuer' }, { status: 400 })
    }
    if (!isIssuerAllowed(issInput)) {
    ...
    const message = error instanceof Error ? error.message : 'EHR launch failed'
